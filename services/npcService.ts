...
export const loadNpcsForMap = async (mapId: MapID, poisByMap: Record<MapID, PointOfInterest[]>): Promise<NPC[]> => {
    ...
    for (const rule of proceduralRules) {
        if (!('npcGenerationPrompt' in rule)) {
            // New role-based generation system
            for (const poiId of rule.poiIds) {
                const poi = poisForMap.find(p => p.id === poiId);
                const roleDefinitions = POI_ROLE_DEFINITIONS[poiId];
                if (poi && roleDefinitions) {
                    for (const roleDef of roleDefinitions) {
                        const promise = generateNpcs(roleDef.generationPrompt, roleDef.count)
                            .then(generatedData => generatedData.map((npcData, index) => {
                                // Overwrite the role generated by the AI with the one from our definition.
                                ...
                                const id = `proc-npc-${mapId}-${poiId}-${roleDef.role.replace(/\s/g, '')}-${index}`;
            ...
            // Old prompt-based generation system
            const promise = generateNpcs(rule.npcGenerationPrompt, rule.count)
                .then(generatedData => {
                    const spawnablePOIs = rule.poiIds.map(id => poisForMap.find(p => p.id === id)).filter((p): p is PointOfInterest => !!p);
                    return generatedData.map((npcData, index) => {
                        ...
                        const id = `proc-npc-${mapId}-${Date.now()}-${index}`;
                        return createNpcFromData(npcData, id, { x, y });
                    });
                });
            generationPromises.push(promise);
